...
namespace scene_gen
{
  ...
  void align(float2 &from, float2 by)
  {
    from.x = ceil(from.x / by.x) * by.x;
    from.y = ceil(from.y / by.y) * by.y;
  }
  void align(float2 &from, float by)
  {
  ...
  void init_scene(Block &_settings, SceneGenerationContext &ctx)
  {
    ...
    ctx.heightmap_size = ctx.settings.get_vec2("heightmap_size", float2(1000, 1000));
    ...
    ctx.full_size = ctx.settings.get_vec2("scene_size", float2(100, 100));
    ctx.grass_field_size = ctx.settings.get_vec2("grass_field_size", float2(1750, 1750));
    ctx.grass_field_size = max(min(ctx.grass_field_size, ctx.heightmap_size), ctx.full_size);
    ctx.center = ctx.settings.get_vec2("scene_center", float2(0, 0));
    ctx.cell_size = ctx.settings.get_vec2("cell_size", float2(150, 150));
    ctx.center3 = float3(ctx.center.x, 0, ctx.center.y);
    ...
    ctx.start_pos = ctx.center - 0.5f * ctx.cell_size * float2(ctx.cells_x, ctx.cells_y);
    ...
    for (int i = 0; i < ctx.cells_x; i++)
    {
      for (int j = 0; j < ctx.cells_y; j++)
      {
        ...
        ctx.cells[id].id = id;
        ctx.cells[id].bbox = AABB2D(ctx.start_pos + ctx.cell_size * float2(i, j), ctx.start_pos + ctx.cell_size * float2(i + 1, j + 1));
        ctx.cells[id].influence_bbox = AABB(float3(ctx.cells[id].bbox.min_pos.x, -10, ctx.cells[id].bbox.min_pos.y),
                                            float3(ctx.cells[id].bbox.max_pos.x, 300, ctx.cells[id].bbox.max_pos.y));
      }
    }

    ctx.biome_map.create(AABB2D(ctx.center - ctx.grass_field_size, ctx.center + ctx.grass_field_size), ctx.biome_map_pixel_size);
    ctx.global_mask.create(float3(ctx.center.x,0,ctx.center.y), ctx.grass_field_size, ctx.biome_map_pixel_size);
  ...
  {
    float3 max_tree_size = float3(0.33f*(c.bbox.max_pos.x - c.bbox.min_pos.x),400,0.33f*(c.bbox.max_pos.y - c.bbox.min_pos.y));
    ...
    return AABB(float3(c.bbox.min_pos.x - max_tree_size.x, min_y, c.bbox.min_pos.y - max_tree_size.z),
                float3(c.bbox.max_pos.x + max_tree_size.x, max_y, c.bbox.max_pos.y + max_tree_size.z));
  }
  ...
  {
    float2 mask_size = ctx.grass_field_size;
    std::function<float(float2 &)> func = [&](float2 &po) -> float
    {
      float3 p0 = float3(po.x + 0.5*mask.get_cell_size(), 0, po.y + 0.5*mask.get_cell_size());
      ...
      for (int i=-1;i<=1;i++)
      {
        for (int j=-1;j<=1;j++)
        {
          float3 p = p0 + float3(i*0.5*mask.get_cell_size(), 0, j*0.5*mask.get_cell_size());
  ...
  uint64_t add_object_blk(Block &b, SceneGenerationContext &ctx)
  {
    ...
    float4x4 transform = b.get_mat4("transform");

    bool place_on_terrain = b.get_bool("on_terrain", false);

    if (place_on_terrain)
    {
      float4 from = transform * float4(0, 0, 0, 1);
      float3 pos = to_float3(from);
      float min_h, float_max_h;
      pos.y = ctx.scene.heightmap->get_height(pos);
      transform(1,3) += (pos.y - from.y);
    }
    bool new_model = true;
    ...
    // get instance AABB
    float3 mn_pos = boxes.empty() ? float3(0,0,0) : boxes[0].min_pos;
    float3 mx_pos = boxes.empty() ? float3(0,0,0) : boxes[0].max_pos;
    for (AABB &box : boxes)
    {
    ...
    int2 c_ij_min = to_int2((float2(mn_pos.x, mn_pos.z) - ctx.start_pos) / ctx.cell_size);
    c_ij_min = max(c_ij_min, int2(0,0));
    c_ij_min = min(c_ij_min, int2(ctx.cells_x-1, ctx.cells_y-1));
    int2 c_ij_max = to_int2((float2(mx_pos.x, mx_pos.z) - ctx.start_pos) / ctx.cell_size);
    c_ij_max = max(c_ij_max, int2(0,0));
    c_ij_max = min(c_ij_max, int2(ctx.cells_x-1, ctx.cells_y-1));
  ...
  void plant_tree(float2 pos, int type, SceneGenerationContext &ctx)
  {
    ...
    int2 c_ij = to_int2((pos - ctx.start_pos) / ctx.cell_size);
    if (c_ij.x >= 0 && c_ij.x < ctx.cells_x && c_ij.y >= 0 && c_ij.y < ctx.cells_y)
    {
      ...
      c.prototypes.back().preplanted_trees.push_back(std::pair<int, float3>(type, p));
...
void GenerationCmdExecutor::execute(int max_cmd_count)
{
  ...
  while (!genCmdBuffer->empty() && cmd_left != 0)
  {
    ...
    case GC_REMOVE_BY_ID:
      ...
      {
        int4 unpacked_id = cmd.args.get_ivec4(i, int4(-1, 0, 0, 0));
        if (unpacked_id.x < 0)
          continue;
        ...
        else
        {
          ...
          {
            auto &ctx = genCtx;
            int2 c_ij_min = to_int2((float2(p.first.min_pos.x, p.first.min_pos.z) - ctx.start_pos) / ctx.cell_size);
            c_ij_min = max(c_ij_min, int2(0,0));
            c_ij_min = min(c_ij_min, int2(ctx.cells_x-1, ctx.cells_y-1));
            int2 c_ij_max = to_int2((float2(p.first.max_pos.x, p.first.max_pos.z) - ctx.start_pos) / ctx.cell_size);
            c_ij_max = max(c_ij_max, int2(0,0));
            c_ij_max = min(c_ij_max, int2(ctx.cells_x-1, ctx.cells_y-1));
            for (int i=c_ij_min.x; i<=c_ij_max.x;i++)
            {
    ...
    case GC_GEN_BIOME_MAP:
      ...
      genCtx.biome_map.set_round(float2(0,0), 100, 150, metainfoManager->get_biome_id_by_name("simple_forest"));
      break;
    ...
    case GC_SET_BIOME_ROUND:
    {
      ...
      if (id != -1)
      {
        ...
        float3 pos = cmd.args.get_vec3("pos");
        if (inner_radius > 0 && outer_radius > inner_radius)
          genCtx.biome_map.set_round(float2(pos.x, pos.z), inner_radius, outer_radius, id);
      }
    }
      break;
