                                 ...
                                 cube_shader({"cubemap.vs", "cubemap.fs"}, {"in_Position"})
{
  ...
  tex = engine::textureManager->create_texture(w, h, GL_RGB8, 1, nullptr, GL_RGB, GL_UNSIGNED_BYTE);
...
Cubemap::~Cubemap()
{
  engine::textureManager->delete_tex(cube);
...
void Cubemap::loadCubemap(std::vector<std::string> faces)
{
  ...
  for (unsigned int i = 0; i < faces.size(); i++)
  {
    ...
    if (i == 0)
    {
      cube = engine::textureManager->create_texture_cube(width, height, GL_RGB8, 1);
