...
namespace visualizer
{
    ...
    void packed_leaf_to_model(PackedLeaf &l, Mesh *m, float2 tc_zw = float2(1,0), bool need_tangent = false);
    void branch_to_model(Branch &b, Mesh *m, bool leaves);
    void packed_branch_to_model(PackedBranch &b, Mesh *m, bool leaves, float precision, float2 tc_zw = float2(1,0), bool need_tangents = false);
    ...
    void get_ring(float3 &start, float3 &dir, float radius, SegmentVertexes &sv, int ring_size, float rel_ring_pos,
                  std::vector<float> &mults, float3 p = float3(1,0,0));
    void get_last_seg_vertexes(Segment &s, SegmentVertexes &sv, int ring_size, float rel_ring_pos, float scale);
    void seg_vertexes_to_model(SegmentVertexes &sv, Mesh *m, float2 tc_zw = float2(1,0), bool need_tangents = false);
