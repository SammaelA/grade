      ...
      for (int xi=0;xi<image_w;xi++)
      {
        ...
        for (int yp=0;yp<spp_a;yp++)
        {
          for (int xp=0;xp<spp_a;xp++)
          {
            ...
            if (neural_sdf_sphere_tracing(sdf, bbox, camera.origin, dir, &p0) && inflated_bbox.contains(p0))
            {
              if (lambert)
              {
                constexpr float h = 0.001;
                cur_grad.clear();
                float ddx = (neural_sdf_get_distance(sdf, p0 + glm::vec3(h,0,0)) - neural_sdf_get_distance(sdf, p0 + glm::vec3(-h,0,0)))/(2*h);
                float ddy = (neural_sdf_get_distance(sdf, p0 + glm::vec3(0,h,0)) - neural_sdf_get_distance(sdf, p0 + glm::vec3(0,-h,0)))/(2*h);
                float ddz = (neural_sdf_get_distance(sdf, p0 + glm::vec3(0,0,h)) - neural_sdf_get_distance(sdf, p0 + glm::vec3(0,0,-h)))/(2*h);
                glm::vec3 n = glm::normalize(glm::vec3(ddx, ddy, ddz));
                color += glm::vec3(1,1,1) * MAX(0.1f, dot(n, light_dir));
              }
              else
