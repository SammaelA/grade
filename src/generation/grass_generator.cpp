...
void GrassGenerator::add_patch(std::vector<Cell> &cells, int cells_x, int cells_y, int cell_id, int type_id)
{
    ...
    //choose parameters
    float2 pos = float2(urand(main_cell.bbox.min_pos.x,main_cell.bbox.max_pos.x),
...
void GrassGenerator::prepare_grass_patches(std::vector<Cell> &cells, int cells_x, int cells_y)
{
    if (cells_x <= 0 || cells_y <= 0 || used_grass_types.empty())
        return;
    float2 cell_size = cells[0].bbox.max_pos - cells[0].bbox.min_pos;
    float2 full_size = float2(cells_x, cells_y)*cell_size;
...
public:
    ...
    GrassGrid(Cell &cell, float plant_av_sz)
    {
        ...
        sz = int2(precision/plant_av_sz*cell_sz) + int2(1,1);
        p_sz = cell_sz/float2(sz);
    ...
    int4 slice(AABB2D &box)
    {
        float2 grdf = (box.min_pos - cell_pos)/p_sz;
    ...
    float2 center(int x, int y)
    {
        return cell_pos + p_sz*float2(x + 0.5, y + 0.5);
    }
...
void GrassGenerator::generate_grass_in_cell(Cell &cell, Field_2d *occlusion)
{
    ...
    for (int patch_id : cell.grass_patches)
    {
        ...
        for (int x = grid_slice.x; x < grid_slice.z; x++)
        {
            for (int y = grid_slice.y; y < grid_slice.w; y++)
            {
                ...
                if (patch.sphere.contains(center))
                {
                    ...
                    {
                        float light = 1/(1 + occlusion->get_bilinear(float3(center.x, 0, center.y)));
                        if (light > 0.005)
                    ...
                    {
                        float2 pos = center + grid.p_sz*float2(urand(-0.5, 0.5), urand(-0.5, 0.5));
        ...
        {
            float2 pos = float2(urand(pb.max_pos.x, pb.min_pos.x),urand(pb.max_pos.y, pb.min_pos.y));
                                            ...
                                            float density, std::vector<std::pair<int, float>> types)
{
    ...
    float2 grid_sz = float2(5,5);
    int2 grid_cnt = int2((cell.bbox.max_pos - cell.bbox.min_pos)/grid_sz);
    grid_sz = (cell.bbox.max_pos - cell.bbox.min_pos)/float2(grid_cnt);
    ...
    for (int i=0;i<grid_cnt.x;i++)
    {
        ...
        {
            float2 pos = cell.bbox.min_pos + grid_sz*float2(i,j);
            float light = 1.0;
            float mask_val = 1.0;
            if (mask)
            ...
            for (int k = 0;k<cnt;k++)
            {
                float2 i_pos = pos + grid_sz*float2(urand(-0.67,0.67),urand(-0.67,0.67));
                float rnd = urand();
...
{ 
  for (auto &v : grass_instances)
  {
    ...
    int2 grass_tex_size = int2(512, 512);
    ...
    for (auto &patch : grass_patches)
    {
        ...
        {
            float3 pos3 = float3(in.pos.x, 0, in.pos.y);
