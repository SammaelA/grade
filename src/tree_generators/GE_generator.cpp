    ...
    for (auto &t : trees)
    {
        ...
        if (t.status == TreeStatus::GROWING)
        {
            ...
            else
            {
                ...
                calc_distance_from_root(t.root, params, float2(0,0));
...
void GETreeGenerator::plant_tree(float3 pos, const TreeTypeData *type)
{
...
void GETreeGenerator::create_leaves(Branch &b, const GETreeParameters &params, int level_from, LightVoxelsCube &voxels)
{
    ...
    {
        Joint &j = *jit;
        float3 dir_x = normalize(jit->pos - prev_jit->pos);
        float3 dir_y, dir_z;
            ...
            j.r < params.base_r * params.leaves_max_r)
        {
            ...
            {
                float3 rd1 = normalize(float3(self_rand(-params.leaves_angle_a, params.leaves_angle_a), 
                                               self_rand(-1, 1), self_rand(-1, 1)));
                rd1 = dir_x*rd1.x + dir_y*rd1.y + dir_z*rd1.z;

                float3 rd2 = normalize(float3(self_rand(-params.leaves_angle_b, params.leaves_angle_b),
                                               self_rand(-1, 1), self_rand(-1, 1)));
...
void GETreeGenerator::create_initial_trunk(Tree &t, const GETreeParameters &params)
{
    ...
    {
        t.root.joints.push_back(Joint(t.pos + float3(0,-1,0), iteration, true));
        for (int i = 0; i < 10; i++)
        {
            float phi = 0.2*PI*(i + self_rand());
            t.root.joints.push_back(Joint(t.pos + float3(0, 0.1*(i+1), 0), iteration, true));
            ...
            b.joints.push_back(Joint(t.pos + 3.0f*float3(sz_x*sin(phi),sz_y,sz_x*cos(phi)), iteration, true));
        }
    }
    else if (params.root_type == 1)
    {
        t.root.joints.push_back(Joint(t.pos + float3(0,-1,0), iteration, true, params.trunk_bonus_radius));
        float dx = 0.05*params.Xm*sz_x;
        float dy = 0.05*params.Xm*sz_y;
        for (int i = 0; i < 4; i++)
        {
            t.root.joints.push_back(Joint(t.root.joints.back().pos + float3(0.1*self_rand(-1,1)*dx, dy, 0.1*self_rand(-1,1)*dx), 
    ...
    {
        t.root.joints.push_back(Joint(t.pos + float3(0,-2,0), iteration, true));
        for (int i = 0; i < 10; i++)
        {
            float phi = 0.2*PI*(i + self_rand());
            t.root.joints.push_back(Joint(t.pos + float3(0, (-2 + 0.03*(i+1))*sz_y, 0), iteration, true));
            t.root.joints.back().childBranches.push_back(Branch(1,t.root.joints.back().pos, iteration,false));
            auto &b = t.root.joints.back().childBranches.back();
            b.joints.push_back(Joint(t.root.joints.back().pos + (float)self_rand(1,5)*sz_x*float3(sin(phi),0,cos(phi)), iteration,false));
            b.joints.push_back(Joint(b.joints.back().pos + float3(self_rand(-0.2,0.2),4*sz_y,self_rand(-0.2,0.2)), iteration, true));
        }
    }
...
void GETreeGenerator::create_universal_initial_trink(Tree &t, const GETreeParameters &params)
{
    ...
    {
        t.root.joints.push_back(Joint(t.pos - float3(0,sz_y*params.L0,0), 0, false, params.R0));
        dL = -sz_y*params.L0/4;        
    }

    for (int i=0;i<4;i++)
    {
        float3 pos = t.root.joints.front().pos + float3(0,(i+1)*dL,0);
    ...
    for (int i=0;i<params.B_cnt;i++)
    {
        ...
        float3 r_pos = t.root.joints.back().pos;
        ...
        float3 b_pos = r_pos + l*float3(sz_x*sin(psi)*cos(phi),sz_y*cos(psi),sz_x*sin(psi)*sin(phi));
...
void GETreeGenerator::convert(Tree &src, ::Tree &dst)
{
    ...
    dst.root->plane_coef = float4(1, 0, 0, -src.pos.x);
    dst.root->id = dst.id;
...
void GETreeGenerator::convert(Tree &src, ::Tree &dst, Branch &b_src, ::Branch *b_dst)
{
    ...
    for (auto it = b_src.joints.begin(); it != b_src.joints.end(); it++)
    {
        ...
        for (Branch &chb : it->childBranches)
        {
            ...
            nb->plane_coef = float4(1, 0, 0, -b_dst->joints.back().pos.x);
            nb->id = dst.id;
...
void GETreeGenerator::calc_distance_from_root(Branch &b, const GETreeParameters &params, float2 start_distance)
{
    if (b.joints.size() < 2)
        return;
    ...
    {
        float3 d = it->pos - prev_it->pos;
        it->distance_from_root = prev_it->distance_from_root + float2(sqrt(SQR(d.x) + SQR(d.z)), abs(d.y));
...
void GETreeGenerator::add_SPCol_points_solid_angle(float3 pos, float3 dir, float r_max, int cnt, float min_psi)
{
    float3 cr, trd;
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        float3 dr = r * cos(psi) * sin(phi) * cr + r * cos(psi) * cos(phi) * trd + r * sin(psi) * dir;
        float3 ps = pos + dr;
        sp_data.add(ps);
    ...
    for (int i = 0; i < max_growth_per_node; i++)
    {
        ...
        for (int j = 0; j < growth_points.size(); j++)
        {
            ...
            if (gp.gType == GrowthType::BRANCHING)
            {
                ...
                float2 planar_dir = float2(sin(phi), cos(phi));
            ...
            if (find_best_pos(voxels, influence_r, start->pos, prev_dir, PI, best_pos, best_occ) && best_pos.x == best_pos.x)
            {
                ...
                float3 best_dir = prev_dir + params.mu * normalize(best_pos - start->pos) +
                                (nu) * tropism(distance_from_trunk, params);
                float3 new_pos = start->pos + params.ro * normalize(best_dir);
...
bool GETreeGenerator::find_best_pos(LightVoxelsCube &voxels, float r, float3 pos, float3 dir, float angle,
                                    float3 &best_pos, float &best_occ)
{
    return sp_data.find_best_pos(voxels, r, pos, dir, angle, best_pos, best_occ);
}

bool GETreeGenerator::SpaceColonizationData::find_best_pos(LightVoxelsCube &voxels, float r, float3 pos,
...
void GETreeGenerator::SpaceColonizationData::remove_close(float3 pos, float r)
{
    if (!active)
       return;
    AABB box = AABB(pos - r * float3(1, 1, 1), pos + r * float3(1, 1, 1));
...
void GETreeGenerator::SpaceColonizationData::add(float3 pos)
{
    if (!active)
        return;
