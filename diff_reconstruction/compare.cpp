                       ...
                       float y_rot = 0, bool render_mask = true)
{
  ...
  for (int i = 0; i < rotations; i++)
  {
    ...
    camera.origin = float3(camera_dist * sin(phi) * cos(psi), camera_y + camera_dist * sin(psi), camera_dist * cos(phi) * cos(psi));
                      ...
                      std::string texture_path, std::string save_dir)
{
  ...
  visualizer::transform(res.first, LiteMath::rotate(float4x4(), PI, float3(0, 1, 0)));
                           ...
                           float phi_min, float phi_max, float psi_min, float psi_max, float jitter)
{
  ...
  for (int i = 0; i < images; i++)
  {
    ...
    camera.target = float3(urand(-jitter, jitter), urand(-jitter, jitter), urand(-jitter, jitter));
    camera.up = float3(0, 1, 0);
    camera.origin = float3(camera_dist * sin(phi) * cos(psi), camera_y + camera_dist * sin(psi), camera_dist * cos(phi) * cos(psi));
...
void building_2_render_mygen_turntable(MitsubaInterface &mi, CameraSettings &camera)
{
    ...
    visualizer::transform(res.first, LiteMath::rotate(float4x4(), PI/2, float3(0,1,0)));
...
void building_6_render_mygen_turntable(MitsubaInterface &mi, CameraSettings &camera)
{
    ...
    visualizer::transform(res_det.first, LiteMath::rotate(float4x4(), PI/2, float3(0,1,0)));
  ...
  /*
    ...
    float3 sz1 = bbox.max_pos - bbox.min_pos;
    float3 sz2 = res_bbox.max_pos - res_bbox.min_pos;
...
void test_3diou_1()
{
    ...
    visualizer::transform(res2.first, LiteMath::rotate(float4x4(), PI, float3(0,1,0)));
...
void render_NGP_turntable(MitsubaInterface &mi, CameraSettings &camera, std::string mygen_name)
{
    ...
    // dgen::shift(model, float3(0,0,0.5));
    ...
    visualizer::transform(ref_model, LiteMath::rotate(float4x4(), -PI / 2, float3(0, 1, 0)));
    ...
    float3 sizes = ref_bbox.max_pos - ref_bbox.min_pos;
