...
def init(base_path, image_w, image_h, spp, mitsuba_variant, render_style, texture_name):
  ...
  scene_dict['model'] = model_silhouette

  scene_dict_textured = {'type': 'scene'}
  scene_dict_textured['integrator'] = integrator
  scene_dict_textured['sensor'] = sensor
  scene_dict_textured['model'] = model_full
  scene_dict_textured['rectangle'] = model_rectangle
  scene_dict_textured['light'] = light

  scene = mi.load_dict(scene_dict)
  scene_textured = mi.load_dict(scene_dict_textured)
  if (render_style == "textured_const"):
    params = mi.traverse(scene_textured)
  else:
    params = mi.traverse(scene)
  context = {
    'scene' : scene,
    'scene_textured' : scene_textured,
    'params' : params, 
...
def render(it, context):
  ...
  if (int(it) == 0):
    ...
    with Image.open("saves/reference_textured.png") as img:
        img.load()
    img = img.convert('RGB')
    img_raw = numpy.asarray(img)
    img_raw = (img_raw/255.0) ** 2.2
    context['img_ref_textured_'+str(i)] = mi.TensorXf(img_raw)
  ...
  for camera_n in range(context['cameras_count']):
    ...
    img_ref_textured = context['img_ref_textured_'+str(camera_n)]
    ...
    img = mi.render(context['scene'], params, sensor = context['camera'], seed=it, spp=1) # image = F_render(scene)
    print(context['camera'])
    if (context['status'] == 'optimization_with_tex'):
      img_textured = mi.render(context['scene_textured'], params, sensor = context['camera'], seed=it, spp=context['spp'])
      loss = 2*context['loss_function'](img, img_ref) + context['loss_function'](img_textured, img_ref_textured)
    else:
  ...
  if (context['status'] == 'optimization_with_tex'):
    ...
    if context['save_intermediate_images'] > 0:
      ...
      mi.util.write_bitmap("saves/res_ref_opt_iter"+str(it)+".png", img_textured)
